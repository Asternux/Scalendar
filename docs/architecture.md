...
- `importers/`、`exporters/`：Excel 适配器位于 Python 边界内。标准工作簿使用 `Courses` 与 `Settings` 两个 Sheet；外部工作簿只在字段和范围足够明确时进入候选状态，不能可靠识别时返回可理解错误。ICS 仍留给 M7。
